...
const localStrategy = require("passport-local"); //ye line allow kr rahi h koie banda account bana sake user name our password ke basis pe
const userModel = require("./users"); //user.js se export jo kiya tha in line 49 vo edhar humne import kr diya . ab humko sab kuch likhna h edhar profile saved stories sab kuch  other than password 
const postModel = require("./posts");
const storyModel = require("./story");
passport.use(new localStrategy(userModel.authenticate())); //bande ko logged in rakh rha h
...
router.post("/register", function (req, res) {
  const user = new userModel({
    username: req.body.username, //jo bhi hum insta pe username dalenge vo username variable me store ho jayega
    email: req.body.email,
    name: req.body.name,
  });

  userModel.register(user, req.body.password).then(function (registereduser) {  //register mera account bana rha h
    passport.authenticate("local")(req, res, function () {
      res.redirect("/profile"); //jab bhi account khulega toh account profile pe pahauch jayega
    });
  }); //yaha tk register ho gaya hai account bann gaya hai
});

router.post(
  "/login",  //username our password ke basis pe login krne ki koshish kro
